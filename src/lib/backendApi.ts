...
export function getPaymentCallbackUrl(): string {
  ...
  if (typeof window !== 'undefined') return `${window.location.origin}/payments/callback`
...
async function parseResponse<T>(response: Response): Promise<T> {
  const data = await response.json().catch(() => null) as ApiEnvelope<T> | ApiErrorEnvelope | null

  if (!response.ok) {
    const message = data && typeof data === 'object'
      ? ('error' in data && typeof data.error === 'string' ? data.error : data.message)
      : null
...
function mapRestaurant(input: ApiRestaurant): Restaurant {
  ...
  return {
    ...
    quantityStep: typeof input.quantity_step === 'number' ? input.quantity_step : 1,
    promoText: input.promo_text ?? null,
    platePrice: moneyOrZero(input.plate_price),
    deliveryFee: moneyOrZero(input.delivery_fee),
    totalMenuItems: typeof input.total_menu_items === 'number' ? input.total_menu_items : null,
  }
}
...
function mapMenuItem(input: ApiMenuItem, restaurantId: string): MenuItem {
  ...
  const categoryId = categoryName.toLowerCase().replace(/\s+/g, '-') || (input.dish_type ?? 'menu')
  ...
  return {
    ...
    maxPortion: typeof input.max_portion === 'number' ? input.max_portion : null,
...
function buildMenuCategories(restaurantId: string, items: MenuItem[]): MenuCategory[] {
...
function mapCart(input: ApiCart | null): Cart {
  ...
  const dishes = input.orders.map((order, index): CartDish => {
    ...
    const items = order.items.map((entry): CartLine => ({
  ...
  return {
    id: String(input.id),
    restaurantId: input.orders.length === 1 ? String(input.orders[0].restaurant_id) : (firstRestaurant ? String(firstRestaurant.id) : null),
    restaurantName: input.orders.length === 1 ? input.orders[0].restaurant.name : (firstRestaurant?.name ?? null),
...
function setDiscoveryCaches(restaurants?: Restaurant[], items?: MenuItem[], zones?: DeliveryZone[]) {
  if (restaurants) setRemoteRestaurants(restaurants)
  if (items) setRemoteMenuItems(items)
  if (zones) setRemoteDeliveryZones(zones)
}

function mapOrderHistoryGroup(input: ApiOrderHistoryGroup): OrderHistoryEntryGroup {
  return {
    id: String(input.id),
...
function mapOrderHistoryEntry(input: ApiOrderHistoryEntry): OrderHistoryEntry {
  ...
  const totalItems = orders.reduce((sum, group) => sum + group.items.reduce((groupSum, item) => groupSum + item.quantity, 0), 0)
...
export async function authenticateTelegramUser(initData: string): Promise<{ token: string, user: AppUser }> {
...
export async function fetchRestaurantsFromBackend(): Promise<Restaurant[]> {
  const result = await apiRequest<ApiRestaurant[]>('/restaurants', { method: 'GET' })
  const restaurants = result.map(mapRestaurant)
  setDiscoveryCaches(restaurants)
  return restaurants
}

export async function fetchRestaurantDetailFromBackend(restaurantId: string): Promise<RestaurantMenuResult> {
  const result = await apiRequest<ApiRestaurantDetail>(`/restaurants/${restaurantId}`, { method: 'GET' })
...
export async function fetchRestaurantMenuFromBackend(restaurantId: string): Promise<RestaurantMenuResult> {
  const [restaurant, menu] = await Promise.all([
    apiRequest<ApiRestaurantDetail>(`/restaurants/${restaurantId}`, { method: 'GET' }),
    apiRequest<ApiMenuItem[]>(`/restaurants/${restaurantId}/menu`, { method: 'GET' }),
  ])
...
export async function fetchSearchCatalogFromBackend(query: string): Promise<SearchCatalogResult> {
  const result = await apiRequest<ApiSearchResult>(`/search?q=${encodeURIComponent(query)}`, { method: 'GET' })
  ...
  return {
    ...
    items: result.dishes.map((dish) => ({
      ...mapMenuItem({
        ...
        supports_fractional_quantity: dish.restaurant.supports_fractional_quantity ?? false,
...
export async function fetchLocationsFromBackend(): Promise<DeliveryZone[]> {
  const result = await apiRequest<ApiLocation[]>('/locations', { method: 'GET' })
  const locations = result.map(mapLocation)
  setDiscoveryCaches(undefined, undefined, locations)
  return locations
}

export async function fetchOrderHistoryFromBackend(token: string): Promise<OrderHistoryEntry[]> {
...
export async function createCartOrderInBackend(token: string, restaurantId: string): Promise<Cart> {
...
export async function addCartItemInBackend(token: string, orderId: string, menuItemId: string, quantity: number): Promise<Cart> {
...
export async function updateCartItemInBackend(token: string, cartItemId: string, quantity: number): Promise<Cart> {
...
export async function deleteCartItemInBackend(token: string, cartItemId: string): Promise<Cart> {
  const result = await apiRequest<ApiCart>(`/cart/items/${cartItemId}`, {
    method: 'DELETE',
    token,
  })
...
export async function deleteCartOrderInBackend(token: string, orderId: string): Promise<Cart> {
...
export async function updateCartLocationInBackend(token: string, locationId: string | null): Promise<Cart> {
  const result = await apiRequest<ApiCart>('/cart/location', {
    method: 'PATCH',
    token,
    body: JSON.stringify({ location_id: locationId ? Number(locationId) : null }),
  })
  return mapCart(result)
}

export async function updateCartInfoInBackend(token: string, note: string): Promise<Cart> {
...
export async function updateCartReferralInBackend(token: string, referralCode: string | null): Promise<Cart> {
...
function mapPaymentSession(input: ApiPaymentInitializeResult, cartId: string): PaymentSession {
  return {
    cartId,
...
export async function initializePaymentInBackend(token: string, input: { cartId: string, email: string, callbackUrl: string }): Promise<PaymentSession> {
  const result = await apiRequest<ApiPaymentInitializeResult>('/payments/initialize', {
    method: 'POST',
    token,
    body: JSON.stringify({
...
export async function verifyPaymentInBackend(token: string, reference: string): Promise<PaymentRecord> {
...
export async function fetchPaymentStatusFromBackend(token: string, reference: string): Promise<PaymentRecord> {
  const result = await apiRequest<ApiPayment>(`/payments/${encodeURIComponent(reference)}`, {
    method: 'GET',
    token,
  })
  return mapPaymentRecord(result)
...
export async function cancelPaymentInBackend(token: string, reference: string): Promise<PaymentRecord> {
